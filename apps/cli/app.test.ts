...
describe("Git Worktree Creation - Windows Compatibility", () => {
	...
	it("should demonstrate Windows mkdir -p compatibility issue", () => {
		...
		const windowsWorkspaceDir =
			"C:\\Users\\user\\.miley\\workspaces\\repo-name";
	...
	it("should show Windows Command Prompt mkdir syntax differences", () => {
		...
		const workspaceCommand = `mkdir -p "C:\\Users\\user\\.miley\\workspaces\\repo-name"`;
	...
	it("should successfully create directories using mkdirSync cross-platform solution", async () => {
		...
		const testPaths = [
			"/tmp/test/workspace",
			"C:\\Users\\user\\.miley\\workspaces\\repo-name",
			"/home/user/.miley/workspaces/project",
	...
	it("should verify the fix replaces problematic execSync calls", async () => {
		...
		const workspaceBaseDir = "/home/user/.miley/workspaces/repo-name";
		mockMkdirSync(workspaceBaseDir, { recursive: true });

		// 2. Fallback path creation (was line 1324)
		const fallbackPath = "/home/user/.miley/workspaces/repo-name/ISSUE-123";
...
describe("Windows Bash Script Compatibility", () => {
	...
	it("should demonstrate Windows bash command compatibility issue", () => {
		...
		// Mock existsSync to simulate miley-setup.sh exists
		mockExistsSync.mockReturnValue(true);

		// Mock Windows Command Prompt behavior where bash is not recognized
		mockExecSync.mockImplementation((cmd: string) => {
			if (cmd.includes("bash miley-setup.sh")) {
				const error = new Error(
		...
		const bashCommand = "bash miley-setup.sh";
	...
	it("should show different shell availability across platforms", () => {
		const testScenarios = [
			{
				platform: "win32",
				command: "bash miley-setup.sh",
				expectedError:
					"'bash' is not recognized as an internal or external command",
			},
			{
				platform: "win32",
				command: "powershell -ExecutionPolicy Bypass -File miley-setup.ps1",
	...
	it("should identify the exact problematic bash execution in app.ts", () => {
		...
		// Line 1294: execSync("bash miley-setup.sh", { ... })
		...
		const problematicCommand = "bash miley-setup.sh";
		const execOptions = {
	...
	it("should successfully execute cross-platform setup scripts", () => {
		...
		const testScenarios = [
			{
				platform: "win32",
				availableScripts: ["miley-setup.ps1"],
				expectedCommand:
					"powershell -ExecutionPolicy Bypass -File miley-setup.ps1",
				description: "Windows with PowerShell script",
			},
			{
				platform: "win32",
				availableScripts: ["miley-setup.bat"],
				expectedCommand: "miley-setup.bat",
				description: "Windows with batch script",
			},
			{
				platform: "win32",
				availableScripts: ["miley-setup.cmd"],
				expectedCommand: "miley-setup.cmd",
				description: "Windows with cmd script",
			},
			{
				platform: "darwin",
				availableScripts: ["miley-setup.sh"],
				expectedCommand: "bash miley-setup.sh",
			...
			{
				platform: "win32",
				availableScripts: ["miley-setup.sh"], // Fallback on Windows
				expectedCommand: "bash miley-setup.sh",
		...
		for (const scenario of testScenarios) {
			...
			const setupScripts = [
				...
				},
				{
					...
					command: "miley-setup.cmd",
					platform: "windows",
				},
				{
					file: "miley-setup.bat",
					command: "miley-setup.bat",
					platform: "windows",
				},
			];
	...
	it("should verify the cross-platform fix replaces hardcoded bash execution", () => {
		// Test that the fix no longer uses hardcoded "bash miley-setup.sh" command
		...
		mockExistsSync.mockImplementation((path: string) => {
			return (path as string).endsWith("miley-setup.ps1");
		});

		mockExecSync.mockImplementation((cmd: string) => {
			if (cmd === "powershell -ExecutionPolicy Bypass -File miley-setup.ps1") {
				return "";
			}
			throw new Error(`Unexpected command: ${cmd}`);
		});
