		...
		try {
			...
			const queryOptions: Parameters<typeof query>[0] = {
				...
				options: {
					...
					// Pass only the superpowers plugin explicitly so its SessionStart hook fires.
					// settingSources discovers plugin skills but NOT hooks. The SDK's plugins
					// option handles hook registration. Only superpowers is passed — loading all
					// 12+ plugins hung the session (NEX-502).
