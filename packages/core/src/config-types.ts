...
import type { Workspace } from "./MileyAgentSession.js";
 ...
 *
 * @param path - Path that may contain ~ prefix (e.g., "~/.miley/repos/myrepo")
...
export interface EdgeWorkerRuntimeConfig {
	/** Miley CLI version (e.g., "1.2.3"), used in /version endpoint */
	version?: string;

	/** Miley home directory - required at runtime */
	mileyHome: string;
	...
	/** The name/handle the agent responds to (e.g., "john", "miley") */
	agentHandle?: string;
 ...
 * - EdgeConfig: Serializable settings from ~/.miley/config.json
